...
counts (with replicate annotation and guide annotations) $\rightarrow$ `gemini.input` $\rightarrow$ `gemini.model` $\rightarrow$ `gemini.score`
...
#### Convergence
After running `gemini_inference`, the resulting convergence rate can be visualized. If the model is divergent, alternative parameters for the priors should be selected until convergence is achieved. This can be done through a form of cross-validation.
