...
#' @param show_inference a logical indicating whether to show the inferred individual or combined values for each gene/gene pair (default T)
#' @param color_x a logical indicating whether to visualize the sample-independent effects for each individual guide or guide pair (default F)
#' @param identify_guides a logical indicating whether to identify guides with unique colors and shapes (default F)
#'
...
gemini_boxplot <- function(Model,
	...
	stopifnot(gene.column.names %in% colnames(Input$guide.pair.annot))
	if(color_x & identify_guides){
		color_x = F
		warning("color_x and identify_guides cannot be concurrently visualized. Defaulting to identify_guides.")
	}
	...
	if (is.null(ginc) | is.null(hjnc)) {
		ginc = Input$guide.pair.annot$rowname[Input$guide.pair.annot[[gene.column.names[1]]] == g &
											  	Input$guide.pair.annot[[gene.column.names[2]]] == nc_gene |
											  	Input$guide.pair.annot[[gene.column.names[1]]] == nc_gene &
											  	Input$guide.pair.annot[[gene.column.names[2]]] == g]
		hjnc = Input$guide.pair.annot$rowname[Input$guide.pair.annot[[gene.column.names[1]]] == h &
											  	Input$guide.pair.annot[[gene.column.names[2]]] == nc_gene |
											  	Input$guide.pair.annot[[gene.column.names[1]]] == nc_gene &
											  	Input$guide.pair.annot[[gene.column.names[2]]] == h]
	}
	...
	df_ginc <- lapply(ginc, function(x) {
		df = data.frame(
			gihj = x,
			gi = strsplit(x, split = Model$pattern_split, fixed = T)[[1]][1],
			hj = strsplit(x, split = Model$pattern_split, fixed = T)[[1]][2],
			D = Input$LFC[x, sample],
			stringsAsFactors = F
		) %>%
			mutate(x_gi = Model$x_seq1[gi]) %>%
			mutate(x_hj = Model$x_seq2[hj]) %>%
	...
	if (color_x) {
		...
		# 		x[which.max(abs(x))])) %>%
		# 	mutate(avg_x = apply(xs, 1, mean, na.rm = T))
		...
		p = ggplot(data = data, aes(
			...
			labs(x = "", y = "Log-fold change", title = sample)
	} else if (identify_guides) {
		# Get guide sequences
		g_seqs <-
			c(names(which(Model$hashes_x$gene_hash1 == g)), names(which(Model$hashes_x$gene_hash2 ==
																			g)))
		h_seqs <-
			c(names(which(Model$hashes_x$gene_hash1 == h)), names(which(Model$hashes_x$gene_hash2 ==
																			h)))
		...
		gg_color_hue <- function(n) {
			...
			pickedcolors = hcl(h = hues, l = 65, c = 100)[1:n]
			pickedcolors = c(pickedcolors, hcl(0, 0, 0))
			return(pickedcolors)
		}
		...
		gg_shape_select <- function(n) {
			...
			if(n > length(allshapes)){
				warning("More guides than available shapes: Not all guides (shapes) may be distinguishable!")
				pickedshapes <- sample(allshapes, size = n, replace = T)
			}else{
				pickedshapes <- sample(allshapes, size = n, replace = F)
			}
			pickedshapes <- c(pickedshapes, 16)
			return(pickedshapes)
		}
		
		p = ggplot(data = data, aes(x = label, y = D)) +
			geom_boxplot(outlier.shape = NA) +
			geom_jitter(
				data = data,
